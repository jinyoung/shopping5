<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['RowHeader','points',]" />
            <wj-flex-grid-cell-template cellType="RowHeader" v-slot="cell">{{cell.row.index + 1}}</wj-flex-grid-cell-template>
            <wj-flex-grid-column binding="points" header="포인트" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
                ...
                <template>
                    <v-card>
                        <v-toolbar
                            ...
                            height="50px"
                        >
                            <div style="color:white; font-size:17px; font-weight:700;">포인트적립 등록</div>
                            <v-spacer></v-spacer>
                            <v-icon
                                color="white"
                                small
                        ...
                        <v-card-text>
                            <PointAccumulation :offline="offline"
                                :isNew="!itemToEdit"
                                :editMode="true"
                                v-model="newValue"
                                @add="append"
