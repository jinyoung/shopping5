<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['RowHeader','연식','배기량','maintenanceHistories',]" />
            <wj-flex-grid-cell-template cellType="RowHeader" v-slot="cell">{{cell.row.index + 1}}</wj-flex-grid-cell-template>
            <wj-flex-grid-column binding="연식" header="연식" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="배기량" header="배기량" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <DetailGrid label="MaintenanceHistories" offline v-if="selectedRow" v-model="selectedRow.maintenanceHistories"/>
        <v-col>
            <v-dialog
                v-model="openDialog"
